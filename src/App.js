...
export default function App() {
  ...
  const { alpha, beta: betaVal } = (() => {
    ...
    const btcR = rawSlice.slice(1)
      .map((d,i) => (d.btc && rawSlice[i].btc && rawSlice[i].btc > 0)
        ? (d.btc - rawSlice[i].btc) / rawSlice[i].btc : null)
      .filter(v => v !== null);
  ...
  useEffect(() => {
    const fetchLive = async () => {
      try { const r=await fetch(`${BACKEND_URL}/api/stats`); const d=await r.json(); setLiveData(d); setLastUpdated(d.lastUpdated); } catch {}
    };
    const fetchNav = async () => {
      try { const r=await fetch(`${BACKEND_URL}/api/nav-history`); const d=await r.json(); setNavHistory(d); } catch {}
    };
    const fetchTrades = async () => {
      try { const r=await fetch(`${BACKEND_URL}/api/closed-trades`); const d=await r.json(); if(d.total) setClosedTrades(d.total.toLocaleString()); } catch {}
    };
  ...
  const minVal = filteredCurve.length ? Math.min(...filteredCurve.map(d=>Math.min(d.nav,d.btc))) : 900;
  const maxVal = filteredCurve.length ? Math.max(...filteredCurve.map(d=>Math.max(d.nav,d.btc))) : 1100;

  const handleSubmit = async () => {
    if (!name||!email) return;
    setSending(true);
    try {
      const res = await fetch("https://formspree.io/f/myklykel",{method:"POST",headers:{"Content-Type":"application/json","Accept":"application/json"},body:JSON.stringify({name,email,message:company})});
      if(res.ok) setSent(true); else alert("Error. Please email us at "+CONTACT_EMAIL);
        ...
        <div style={{maxWidth:1100,margin:"0 auto"}}>
          ...
          <div style={{marginTop:24,background:"rgba(192,122,138,0.06)",border:"1.5px solid rgba(192,122,138,0.25)",borderRadius:14,padding:"16px 20px",display:"flex",alignItems:"flex-start",gap:12}}>
            ...
            <div style={{fontSize:13,color:"#7a5060",lineHeight:1.7}}>
              <strong style={{color:"#c07a8a"}}>Important risk notice:</strong> We highly encourage users to follow Elevano Capital <strong>1-to-1</strong> using <strong>Fixed Ratio</strong> copy mode. Do not add extra leverage beyond our own position sizing, as this can lead to <strong>liquidation in case of severe market movements</strong>. Copy us as-is — that's how the strategy is designed to work.
            </div>
          </div>
        </div>
      </section>
            ...
            <div className="fade-up" style={{background:"rgba(192,122,138,0.04)",border:"1.5px solid rgba(192,122,138,0.25)",borderRadius:16,padding:"24px"}}>
              <div style={{display:"flex",gap:16,alignItems:"flex-start"}}>
                ...
                <div style={{width:"100%"}}>
                  ...
                  <div style={{fontSize:13,color:"#7a5060",lineHeight:1.65,marginBottom:12}}>Set your investment amount, select <strong>Fixed Ratio</strong> copy mode, and keep leverage at <strong>1-to-1</strong>. Click <strong>Copy Now</strong>. The <strong>Trailing Stop</strong> boxes can be left unticked — stop loss management is already handled at the strategy level.</div>
                  ...
                  <div style={{display:"flex",alignItems:"flex-start",gap:8,background:"rgba(192,122,138,0.08)",borderRadius:10,padding:"12px 14px"}}>
                    ...
                    <div style={{fontSize:12,color:"#c07a8a",lineHeight:1.6}}><strong>Important:</strong> Select <strong>Fixed Ratio</strong> copy mode. Do not change the leverage — adding leverage can lead to <strong>liquidation</strong>.</div>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div style={{maxWidth:800,margin:"0 auto"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:2}}>
            {[
              ...
              {q:"How does copy trading work?",a:"Go to Bybit Copy Trading, find Elevano Capital, enter the amount you want to allocate, and click Copy. Select Fixed Ratio copy mode. Do not change the leverage. You can start, pause, or stop copying at any time. ⚠️ Copy 1-to-1 only."},
